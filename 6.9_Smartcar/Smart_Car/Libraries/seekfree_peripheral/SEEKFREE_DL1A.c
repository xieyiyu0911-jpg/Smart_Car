...
{
    uint16 xdata j=DL1A_SOFT_IIC_DELAY;
    while(j--);
}
...
// 参数说明     index           索引
// 参数说明     type            类型值
// 返回参数     uint8 xdata           是否成功 0-成功 1-失败
...
{
    uint32 xdata macro_period_ns = calc_macro_period(vcsel_period_pclks);
...
{
    uint32 xdata ls_byte = 0;
    uint16 xdata ms_byte = 0;
...
{
    uint8 xdata reg_buffer[2];
    uint16 xdata reg16_buffer = 0;
...
// 返回参数     uint8 xdata           操作是否成功 0-成功 1-失败
// 使用示例     dl1a_get_vcsel_pulse_period(DL1A_VCSEL_PERIOD_PER_RANGE);
// 备注信息     在 PCLKs 中获取给定周期类型的 VCSEL 脉冲周期
//-------------------------------------------------------------------------------------------------------------------
...
// 返回参数     uint8 xdata           操作结果 0-成功 1-失败
...
{
    uint8 xdata return_state = 0;
    uint8 xdata data_buffer[3];
    uint16 xdata dat = 0;
    uint32 xdata used_budget_us;
    uint32 xdata final_range_timeout_us;
    uint16 xdata final_range_timeout_mclks;
...
{
    uint8 xdata data_buffer[3];
    uint16 xdata limit_mcps_16bit = (limit_mcps * (1 << 7));
...
{
    uint32 xdata measurement_timing_budget_us = 0;
    uint8 xdata stop_variable = 0;
    uint8 xdata return_state = 0;
    uint8 xdata reg_data_buffer = 0;
    uint8 xdata ref_spad_map[6];
    uint8 xdata data_buffer[7];
    uint8 xdata i = 0;
