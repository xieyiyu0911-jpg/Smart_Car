 ...
 */
 ...
 extern float xdata SpeedMeasure_R;// 右轮实际速度
 ...
 extern volatile int xdata Motor_Speed_Left[7];    // 左轮编码器原始数组
 extern volatile int xdata Motor_Speed_Right[7];   // 右轮编码器原始数组
 extern volatile float xdata Yaw_Angle;            // 环岛偏航角积分
 extern volatile float xdata Turn_Output;          // 转向输出（显示用）
 extern volatile uint16 xdata conservation;        // 保护标志
 extern Round_State_TypeDef Round_State;           // 环岛状态
 extern uint8 Round_Direction;                     // 环岛方向
 extern float xdata Round_Pre_Distance;                  // 预处理里程
 extern float xdata Round_Exit_Distance;                 // 出环后里程
...
//	seekfree_assistant_receive = seekfree_assistant_receive_callback;// 回调函数
		...
		//无线转串口
		// 注册上位机发送回调
	seekfree_assistant_transfer = wireless_uart_send_buff;// 回调函数
	
		// 注册上位机接收回调
	seekfree_assistant_receive = wireless_uart_read_buff;// 回调函数
