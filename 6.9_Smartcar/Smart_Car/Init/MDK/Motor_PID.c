...
// Kp2 二次项增益规则。主要由 |E| 幅值驱动，EC 依赖较 Kp 弱，避免与 P1 产生强指抗。
// 大误差时提供非线性 boost 应对急弯，小误差时归零（二次项自身已随误差缩小而衰减）。
...
// ========== 环岛处理变量 ==========
  ...
  // 里程计数
  float xdata Round_Pre_Distance = 0;                  // 预处理阶段里程
  float xdata Round_Exit_Distance = 0;                 // 出环后里程
...
//           = [2] + result_Kp × [0]
//  - result_Kp（双线性插值输出）：[-3, 3]
//  - FUZZY_KP_SCALE = [0]，默认 0.2 → delta_Kp ∈ [-0.6, +0.6]
//  - 假设 Servo_P1 = 1.3，则 fuzzy 调节后：[0.7, 1.9]
//  - 代码硬限幅：[0.8, 4.0]
...
//          = [3] + result_Kd × [1]
//  - result_Kd（双线性插值输出）：[-3, 3]
//  - FUZZY_KD_SCALE = [1]，默认 2.33 → delta_Kd ∈ [-6.99, +6.99]
//  - 假设 Servo_D = 10.0，则 fuzzy 调节后：[3.0, 17.0]
//  - 代码硬限幅：[5.0, 25.0]
...
//           = [4] + result_Kp2 × [6]
//  - result_Kp2（双线性插值输出）：[0, 2]（规则表里没有负值）
//  - FUZZY_KP2_SCALE = [6]，默认 0.001 → delta_Kp2 ∈ [0, +0.002]
//  - 假设 Servo_P2 = 0.004，则 fuzzy 调节后：[0.004, 0.006]
//  - 代码硬限幅：[0.0, 0.012]
