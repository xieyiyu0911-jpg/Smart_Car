...
static uint8            xdata seekfree_assistant_buffer[SEEKFREE_ASSISTANT_BUFFER_SIZE] = {0};              		// 数据存放数组
...
{
    uint8 xdata temp_sum;
    uint32 xdata read_length;
    seekfree_assistant_parameter_struct *receive_packet;

    // 这里使用uint32进行定义，目的是为了保证数组四字节对齐
    uint32 xdata temp_buffer[SEEKFREE_ASSISTANT_BUFFER_SIZE / 4];
