...
//			Tiaocan_Config = seekfree_assistant_parameter[6]; 

//      if(Tiaocan_Config == 1)//调角度环模式
//			{				
//				Differential_Speed_Control(Turn_Output);// 差速分配
				
//				Motor_PID(SpeedTarget_L,motor_speed_L,1.9,2.1,0.05,Left,Turn_Output);// 左轮速度环PID //5,2.5,1.25/1.4,0.8,0/3,1.5,0.3/1.4/0.8
//				Motor_PID(Target_Right1,motor_speed_R,1.2,2,0.2,Right,Turn_Output);// 右轮速度环PID //4.8,2.4,1.25/1.7,0.6,0/2.5,0.8,0.2/1.7，0.8
	//1.5,0.91,0.2
	//4.2,1.32,0
	//3.4,1.32,0
	
	
				Motor_PID(SpeedTarget_L,motor_speed_L,seekfree_assistant_parameter[0],seekfree_assistant_parameter[1],seekfree_assistant_parameter[2],Left,Turn_Output);// 左轮速度环PID //5,2.5,1.25/1.4,0.8,0/3,1.5,0.3/1.4/0.8
				Motor_PID(Target_Right1,motor_speed_R,seekfree_assistant_parameter[3],seekfree_assistant_parameter[4],seekfree_assistant_parameter[5],Right,Turn_Output);// 右轮速度环PID //4.8,2.4,1.25/1.7,0.6,0/2.5,0.8,0.2/1.7，0.8
			
//				conservation = PID_Conservation(Result_L,Result_Middle_M_L,Result_Middle_M_R,Result_R);// 保护判断
//			}

//		else if(Tiaocan_Config == 0)//调速度环模式
//		{
//				Motor_PID(SpeedTarget_L,motor_speed_L,seekfree_assistant_parameter[0],seekfree_assistant_parameter[1],seekfree_assistant_parameter[2],Left,Turn_Output);// 左轮速度环PID //5,2.5,1.25/1.4,0.8,0/3,1.5,0.3/1.4/0.8
//				Motor_PID(Target_Right1,motor_speed_R,seekfree_assistant_parameter[3],seekfree_assistant_parameter[4],seekfree_assistant_parameter[5],Right,Turn_Output);// 右轮速度环PID //4.8,2.4,1.25/1.7,0.6,0/2.5,0.8,0.2/1.7，0.8
//		}
